...
playground_sizes=(10000 14142 17320 20000 22360 24494 26458 28284 30000 31622 33166 34640)        # Playground sizes
...
for task in $(seq 1 12) ; do
  
    size=${playground_sizes[$((task - 1))]}

    mpirun --map-by node --bind-to socket -np 1 $exec_dir/main.x -i -k $size -f "playground_${size}.pgm"

    for run in $(seq 1 5); do
        output=$(mpirun --map-by node --bind-to socket -np $task $exec_dir/main.x -r -f "playground_${size}.pgm" -e $e -n $n -s $s)
