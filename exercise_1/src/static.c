...
void blocking_static_evolution(unsigned char *playground, int width, int height, int block_height, int tot_steps, int snap_step, int rank, int size) {
    // Allocate buffers
    unsigned char *row_buffer = (unsigned char *)malloc(2 * width * sizeof(unsigned char));
    unsigned char *halo_top = row_buffer;
    unsigned char *halo_bottom = row_buffer + width;
    ...
    for (int step = 0; step < tot_steps; step++) {
        ...
        // Compute the next state using the frozen playground
        #pragma omp parallel for collapse(2) schedule(static)
        for (int row = 0; row < block_height; row++) {
            for (int col = 0; col < width; col++) {
                ...
                // Update cell in the next playground
                next_playground[cell_index] = ((playground[cell_index] == ALIVE && (alive_neighbors == 2 || alive_neighbors == 3)) ||
                                               (playground[cell_index] == DEAD && alive_neighbors == 3)) ? ALIVE : DEAD;
            }
        }
        ...
        // Save snapshot 
        if (snap_step > 0 && step % snap_step == 0) {
            save_snapshot(playground, width, height, block_height, step, rank, size);
        }

        // Debug information for rank 0
        if (rank == 0 && step % 100 == 0) {
            printf("Step %d completed\n", step);
        }
    }
    ...
    // Save final snapshot
    if (snap_step == 0 || (snap_step > 0 && tot_steps % snap_step != 0)) {
        save_snapshot(playground, width, height, block_height, tot_steps, rank, size);
    }
